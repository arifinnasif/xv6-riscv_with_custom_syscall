...
scheduler(void)
{
  ...
  for(;;){
    ...
    // find min ticket
    int min_ticket = pow(2, 32) - 1;
    for(p = proc; p < &proc[NPROC]; p++) {
      acquire(&p->lock);
      if(p->state == RUNNABLE) {
        if(p->current_ticket < min_ticket) min_ticket = p->current_ticket;
      }
      release(&p->lock);
    }
    ...
    for(p = proc; p < &proc[NPROC]; p++) {
      acquire(&p->lock);
      if(min_ticket<0) {
        if(p->state == RUNNABLE) total_ticket_count += pow(2, p->current_ticket - min_ticket);
      } else {
        if(p->state == RUNNABLE) total_ticket_count += pow(2, p->current_ticket);
      }

      release(&p->lock);
    }

    if(total_ticket_count == 0 || min_ticket < 0) {
      for(p=proc; p<&proc[NPROC]; p++) {
        // printf("here\n");
        acquire(&p->lock);
        if(p->state == RUNNABLE) {
    ...
    for(p = proc; p < &proc[NPROC]; p++) {
      ...
      if(p->state == RUNNABLE) {
        if(min_ticket<0) {
          cummulitive_ticket_sum += pow(2, p->current_ticket - min_ticket);
        } else {
          cummulitive_ticket_sum += pow(2, p->current_ticket);
        }
